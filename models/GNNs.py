...
class GNN(nn.Module):
    ...
    def __init__(self, num_layer, gnn_layer, init_emb, num_hop1_edge, max_edge_count, max_hop_num, max_distance_count,
                 ...
                 residual=False, use_rd=False, wo_peripheral_edge=False, wo_peripheral_configuration=False, with_kn_configuration=False,
                 drop_prob=0.1):
        ...
        self.with_kn_configuration = with_kn_configuration
        if self.JK == "concat":
        ...
        if with_kn_configuration:
            self.pnw = nn.Parameter(torch.rand(1), requires_grad=True)
    ...
    def reset_parameters(self):
        ...
        if self.with_kn_configuration:
            nn.init.normal_(self.pnw)

    def forward(self, data):
        edge_index, edge_attr, batch = data.edge_index, data.edge_attr, data.batch
        if "kn_attr" in data:
            kn_attr = data.kn_attr
        else:
            kn_attr = None
        ...
        kn_attr_final = torch.zeros([num_nodes, self.K, self.output_dk], device=x.device, dtype=x.dtype)
        if self.with_kn_configuration and kn_attr is not None:
            # repeating kn_attr across two dimensions to be of intended N * K * H size
            kn_attr_duplicated = (torch.transpose(kn_attr.repeat(self.output_dk, 1), 0,1)
                                  .unsqueeze(1).repeat(1, self.K, 1))
            kn_attr_final += torch.sigmoid(self.pnw) * kn_attr_duplicated
        ...
        for l in range(self.num_layer):
            ...
            h = self.gnns[l](h_list[l], edge_index, edge_attr, pe_attr, peripheral_attr, kn_attr_final)
            h = self.norms[l](h)
...
class GNNPrime(nn.Module):
    ...
    def __init__(self, num_layer, gnn_layer, init_emb, num_hop1_edge, max_edge_count, max_hop_num, max_distance_count, num_l1_layer=1,
                 JK="last", norm_type="batch", virtual_node=True,
                 residual=False, use_rd=False, wo_peripheral_edge=False, wo_peripheral_configuration=False,
                 drop_prob=0.1):
    ...
    def forward(self, data):
        ...
        for l in range(self.num_l1_layer):
            if self.virtual_node:
                h_list[l] = h_list[l] + virtualnode_embedding[batch]
            h = self.khop_gnns[l](h_list[l], edge_index, edge_attr, pe_attr, peripheral_attr)
            h = self.norms[l](h)
            h = self.dropout(h)
            if self.residual:
                h = h + h_list[l]
