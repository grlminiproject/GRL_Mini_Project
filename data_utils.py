...
def extract_multi_hop_neighbors(data, K, max_edge_attr_num, max_hop_num,
    ...
    if kernel == "gd":
        ...
        for i in range(1, len(adj_list)):
            ...
            adj_list[i] = adj_
        # create K-hop edge with shortest path distance kernel
        final_adj = exist_adj
    ...
    peripheral_edge_attr, peripheral_configuration_attr = get_peripheral_attr(adj_list, edge_attr_adj, max_hop_num,
                                                                              ...
                                                                              max_distance_count)
    kn_attr = get_kn_attr(edge_attr_adj, final_adj, x)
    ...
    data.kn_attr = kn_attr
    return data


def get_kn_attr(adj_list, k_adj_list, x):
    """Compute peripheral information for each node in graph
    (,,,)
    """
    neighbor_sums = torch.matmul(adj_list, x.type(torch.LongTensor))
    kn_sums = torch.matmul(k_adj_list.type(torch.LongTensor), neighbor_sums)
    if len(kn_sums.size()) > 1:
        kn_sums = torch.matmul(kn_sums, torch.ones(kn_sums.size()[1]).type(torch.LongTensor))
    return kn_sums
