...
class KPGINConv(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_attr, pe_attr=None, peripheral_attr=None, kn_attr_final=None):
        ...
        # add information about sums of neighbors of own k-hop neighbors
        if kn_attr_final is not None:
            x_n = x_n + kn_attr_final
        x = x_n + (1 + self.eps) * x
        x = x.permute(1, 0, 2)
